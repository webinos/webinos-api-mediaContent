...
(function () {
  ...
  var UPnPMediaServerModule = require('./mediacontent_upnpmediaserver.js');
  ...
  WebinosMediaContentModule = function (rpc, params, config) {
      ...
      this.httpHandler = (function (request, response) {
          ...
          if (ref) {
              ...
              if (process.platform !== 'android') {
                  filePath = this.thumbs.getThumbLocalPath(filePath);
                  if(filePath){
                  ...
                  mediaContentImpl.getThumb(filePath, function(err, thumb) {
                      if (err) {
                          response.statusCode = 404;
                          response.end("404 Not Found");
                          return;
                      }
                      var thumbBuf = new Buffer(thumb);
                      response.writeHead(200, {
                          'Content-Type' : 'image/jpeg',
                          'Content-Length' : thumbBuf.length
                      });
                      response.end(thumbBuf);
                  });
              }
          }
      }).bind(this);
  ...
  WebinosMediaContentModule.prototype.init = function (register, unregister) {
      ...
      new UPnPMediaServerModule(register, unregister, this.rpc, this.params);
  };
