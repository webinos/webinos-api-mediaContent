...
(function () {
  ...
  var MediaType = require("./media_types.js"),
    ...
    fs = require("fs"),
    WebinosMediaContentService, WebinosMediaContentModule,
  ...
  WebinosMediaContentModule.prototype.init = function (register, unregister) {
    register(new WebinosMediaContentService(this.rpc, this.params, this.config, this.links));
  };

  WebinosMediaContentModule.prototype.httpHandler = function (request, response) {
  ...
  WebinosMediaContentService = function (rpcHandler, params, config, links) {
  ...
  WebinosMediaContentService.prototype.getLocalFolders = function (params, successCB, errorCB) {
    ...
    if (os.type().toLowerCase() === "linux") {
      ...
      music = path.resolve(process.env.HOME, "Music");
      video = path.resolve(process.env.HOME, "Movies");
      pictures = path.resolve(process.env.HOME, "Pictures");
  ...
  WebinosMediaContentService.prototype.getLink = function (params, successCallback, errorCallback) {
    this.findItem(params, function (item) {
      var ref = uuid.v4();
      this.links[ref] = item.itemURI;
      var link = url.format(
        { protocol : "http"
        , hostname : this.config.http.hostname
