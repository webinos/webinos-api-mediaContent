...
(function () {
  ...
  function itemInfo(id, info) {
    var i, mediaItem, listSend = [], title, count = 0, result, current;
  ...
  MediaSource = function () {
    ...
    this.getFolders = function (successCB, errorCB) {
      var mediaFolder, i, list = [], stats, mediaFolders;
      ...
      for (i = 0; i < mediaFolders.length; i = i + 1) {
        ...
        if (stats.isDirectory()) {
          ...
          mediaFolder.modifiedData = stats.ctime;
    ...
    this.findItems = function (successCB, errorCB, folderId, filter, sortMode, count, offset) {
      function success(list) {
        var i;
        for (i = 0; i < list.length; i = i + 1) {
          fetchMediaInfo(list[i].folderURI, list[i].id, itemInfo);
        }
      }
      this.getFolders(success, errorCB);
    };
    /**
     ...
     * @errorCB
     */
    this.updateItemsBatch = function (items, successCB, errorCB) {
    };
  };
  ...
  WebinosMediaContentModule.prototype.getLocalMediaSource = function (successCB, errorCB) {
    var value = new MediaSource;
    console.log(value)
    successCB(value);
  };
